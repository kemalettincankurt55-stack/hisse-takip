...
export const getUpcomingEvents = (daysAhead = 45): EconomicEvent[] => {
  ...
  for (let m = 0; m < 3; m++) {
    ...
    if (nfp >= now && nfp <= horizon)
      events.push({ id: `nfp-${iso(nfp)}`, date: iso(nfp), time: '15:30', name: 'ABD Tarım Dışı İstihdam', country: 'US', impact: 'high' });
  }

  // ABD Haftalık İşsizlik Başvuruları — her Perşembe (orta etki)
  let thu = nextWeekday(now, 4);
  while (thu <= horizon) {
    events.push({ id: `claims-${iso(thu)}`, date: iso(thu), time: '15:30', name: 'ABD İşsizlik Maaşı Başvuruları', country: 'US', impact: 'medium' });
    thu = new Date(thu);
  ...
  if (tufe <= horizon)
    events.push({ id: `tufe-${iso(tufe)}`, date: iso(tufe), time: '10:00', name: 'TÜİK Enflasyon (TÜFE)', country: 'TR', impact: 'high' });

  // TCMB Faiz Kararı (PPK) — ayda bir, ~ayın 24'ü civarı (yüksek etki, yaklaşık)
  const tcmb = nudgeToWeekday(nextNthOfMonth(now, 24));
  if (tcmb <= horizon)
    events.push({ id: `tcmb-${iso(tcmb)}`, date: iso(tcmb), time: '14:00', name: 'TCMB Faiz Kararı', country: 'TR', impact: 'high', approximate: true });

  // Fed Faiz Kararı (FOMC) — ~6 haftada bir, ~ayın 18'i civarı (yüksek etki, yaklaşık)
  const fed = nudgeToWeekday(nextNthOfMonth(now, 18));
  if (fed <= horizon)
    events.push({ id: `fed-${iso(fed)}`, date: iso(fed), time: '21:00', name: 'Fed Faiz Kararı (FOMC)', country: 'US', impact: 'high', approximate: true });
